...
int main()
{
	...
	// Don't need to delete this at the end bc the memory is allocated directly on the stack frame; if using new keyword, then you need to delete it later
	MyPoint point1(5, 7, 1.9823412);
	MyPoint point2(1, 2, 3);
	MyPoint point3(4, 5, 6);
	point2.print_coords();

	vector<MyPoint> points; // like arraylist in java
	...
	for (const auto &point : points) // auto: c++ will compute the type of point for you. means same thing as const MyPoint
	{
		if (point.get_x() > 2)
		{
