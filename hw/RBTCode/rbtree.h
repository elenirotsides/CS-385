...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value)
    {
        ...
        if (find(key) != end())
        {
            ...
            warning << "Attempt to insert duplicate key '"<< key << "'.";
            throw tree_exception(warning.str());
        }

        Node<K,V> *newNode = new Node<K,V>(key, key_value.second);
...
private:
    ...
    void delete_tree(Node<K, V> *n)
    {
        ...
        if (n != nullptr)
        {
            delete_tree(n->left);
            delete_tree(n->right);
            delete n; // deletes n on every recursive call
        }
    }
    ...
    void insert_fixup(Node<K, V> *z)
    {
        // TODO Done
        /**
         * Implemented algorithm described in the Red-Black Trees.txt file provided to us
         * 
         */
        Node<K, V> *y = nullptr;
        while (z->parent != nullptr && z->parent->color == RED){
            if (z != nullptr && z->parent == z->parent->parent->left){
    ...
    void left_rotate(Node<K, V> *x)
    {
        // TODO Done
        /**
         * Implemented algorithm described in the Red-Black Trees.txt file 
         * and the CLRS textbook pages that were provided to us
         * 
         */
        Node<K, V> *y = x->right;
        x->right = y->left;
    ...
    int height(Node<K, V> *node) const
    {
        ...
        /**
         * Recursively calculates the height of a tree by its nodes, 
         * then adds 1 to account for the root.
         * 
         */
        if (node != nullptr)
        {
        ...
        return -1; // if node is nullptr, then there is no tree so height is -1
    }
    ...
    size_t leaf_count(Node<K, V> *node) const
    {
        ...
        // recursively calculates the leaf count
        if (node != nullptr)
        {
        ...
        return 0; // no leafs exist if the node is nullptr 
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const
    {
        ...
        /**
         * Recursively calculates the internal node count
         * There are three cases to consider; when both children are nodes,
         * when the left node's child is a nullptr, or when the right node's 
         * child is a nullptr.
         * 
         */
        int count = 0;
        if (node != nullptr)
        {
            ...
            if (node->left != nullptr && node->right != nullptr)
            {
                count = 1 + internal_node_count(node->left) + internal_node_count(node->right);
            }
            else if (node->left == nullptr && node->right != nullptr)
            {
                count = 1 + internal_node_count(node->right);
            }
            else if (node->left != nullptr && node->right == nullptr)
            {
                count = 1 + internal_node_count(node->left);
            }
        }
    ...
    int diameter(Node<K, V> *node) const
    {
        ...
        // done in one line; probably not the best way to do this in terms of readability, but it makes sense to us ... LOL
        return max(max(diameter(node->left), diameter(node->right)), height(node->left) + height(node->right) + 2);
    }
    ...
    size_t null_count(Node<K, V> *node) const
    {
        ...
        /**
         * Recursively calculates the null count. There are 5 cases to consider:
         * 1. The node is a nullptr, return 1
         * 2. Both left node and right node are nullptrs, return 2
         * 3. Left node is a nullptr, return 1 and add recursive call to right node
         * 4. Right node is a nullptr, return 1 and add recursive call to left node
         * 5. No children are nullptrs, recursively call function on left node and again on right node
         * 
         */
        if (node == nullptr) {
            return 1;
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const
    {
        ...
        // recursively sums the levels, stops when node is a nullptr (when level no longer exists)
        if (node != nullptr){
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const
    {
        ...
        // recursively sums the null levels, stops when node is a nullptr (when level no longer exists)
        if (node == nullptr){
            return level;
        }
        else {
